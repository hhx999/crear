...
<body>
  ...
  <div style="border-top: 2px solid #4CAF50;display: inline-block;width: 92%;"></div>
    <form action="" method="post" name="crearLineaCreditos" enctype="multipart/form-data">
      ...
      <select name="form_tipo_id">
        @foreach($lineas as $linea)
        <option value="{{$linea->id}}">{{$linea->nombre}}</option>
        @endforeach
      </select>
      <br>
      <label>Bases y condiciones</label>
    	<input type="file" name="basesCondiciones" class="w3-input">
    	<label>Formulario</label>
      <input type="file" name="formulario" class="w3-input">
      <br>
